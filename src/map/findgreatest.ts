function findTheGreatestNumber(arr: number[]): number {
    const found: number | undefined = arr.find(num => num > 10)
    if (found === undefined) {
...
function findAllNumbersGreaterThan(arr: number[]): number[] {
    return arr.filter(num => num > 10);
}

const findFirstGreaterThan10: number[] = [3, 8, 10, 11, 17, 5]
